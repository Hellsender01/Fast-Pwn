...
class tube(Timeout, Logger):
    ...
    def recvn(self, numb, timeout=default):
        ...
        with self.countdown(timeout):
            while (
                ...
                and self._fillbuffer(self.timeout)
            ):
                pass
    ...
    def recvline_endswith(self, delims, keepends=False, timeout=default):
        ...
        return self.recvline_pred(
            lambda line: any(map(line.endswith, delims)),
            keepends=keepends,
            timeout=timeout,
        )
    ...
    def recv_libc_leak(self, offset):
        """recv_leak() -> int
    ...
    def sendpayload(self, offset, payload):
        """send payload with cylic(offset)
    ...
    def connected(self, direction="any"):
        ...
        try:
            ...
            raise KeyError(
                "direction must be in %r" % sorted(self.connected_directions)
            )
        else:
    ...
    def make_wrapper(func):
        ...
        wrapperb.__doc__ = (
            "Same as :meth:`{func.__name__}`, but returns a bytearray".format(func=func)
        )
        wrapperb.__name__ = func.__name__ + "b"
        wrapperS.__doc__ = (
            "Same as :meth:`{func.__name__}`, but returns a str, "
