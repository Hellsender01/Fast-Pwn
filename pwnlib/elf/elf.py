...
class ELF(ELFFile):
    ...
    def __init__(self, path, checksec=True):
        ...
        if self.arch == "mips":
            ...
            if mask(flags, E_FLAGS.EF_MIPS_ARCH_32) or mask(
                flags, E_FLAGS.EF_MIPS_ARCH_32R2
            ):
                pass
            elif mask(flags, E_FLAGS.EF_MIPS_ARCH_64) or mask(
                flags, E_FLAGS.EF_MIPS_ARCH_64R2
            ):
                self.arch = "mips64"
                self.bits = 64
    ...
    def address(self, new):
        ...
        for begin, end, data in self.memory:
            memory.addi(update(begin), update(end), data)
    ...
    def _patch_elf_and_read_maps(self):
        ...
        if sc is None:
            log.error(
                "Cannot patch /proc/self/maps shellcode into %r binary", self.arch
            )
        ...
        if not seg:
            log.warn_once(
                "Could not inject code to determine memory mapping for %r: Not enough space",
                self,
            )
            return {}
    ...
    def _populate_plt(self):
        ...
        with context.local(arch=self.arch, bits=self.bits, endian=self.endian):
            for section in (plt, plt_got, plt_sec, plt_mips):
                ...
                res = emulate_plt_instructions(
                    self, dt_pltgot, section.header.sh_addr, section.data(), inv_symbols
                )
    ...
    def libc_start_main_return(self):
        ...
        if self.arch in ["arm", "thumb"]:
            ...
            log.error(
                "Unsupported architecture %s in ELF.libc_start_main_return", self.arch
            )
            return 0

        code = self.disasm(
            self.symbols["__libc_start_main"], self.functions["__libc_start_main"].size
        )
        ...
        calls = [
            ...
            if set(line.split()) & call_instructions
        ]
    ...
    def _populate_memory(self):
        load_segments = list(
            filter(lambda s: s.header.p_type == "PT_LOAD", self.iter_segments())
        )
    ...
    def disasm(self, address, n_bytes):
        ...
        return disasm(
            self.read(address, n_bytes), vma=address, arch=arch, endian=self.endian
        )
    ...
    def section_addr(self, name, offset=0):
        ...
        """
        if name not in self._section_name_map:
            return None
    ...
    def find_bin_sh(self):
        """
        Returns:
            Address of bin_sh string.
        """
        try:
            bin_sh_addr = next(self.search(b"/bin/sh"))
    ...
    def checksec(self, banner=True, color=True):
        ...
        res.extend(
            [
                ...
                "Stack:".ljust(10)
                + {True: green("Canary found"), False: red("No canary found")}[
                    self.canary
                ],
                "NX:".ljust(10)
                + {
