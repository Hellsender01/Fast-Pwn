...
class ROP(object):
    ...
    def setRegisters(self, registers):
        ...
        for num_gadgets in range(len(registers)):
            for combo in itertools.combinations(
                sorted(best_gadgets.values(), key=repr, reverse=True), 1 + num_gadgets
            ):
    ...
    def generatePadding(self, offset, count):
        ...
        alphabet = b"".join(
            packing.p8(c)
            for c in bytearray(string.ascii_lowercase.encode())
            if c not in self._badchars
        )
            ...
            # broken down into pointer-width blobs.
                ...
                for i, argument in enumerate(stackArguments):
                    ...
                    if isinstance(argument, NextGadgetAddress):
                        ...
                        description = self.describe(argument) or "arg%i" % (
                            i + len(registers)
                        )
    ...
    def _srop_call(self, resolvable, arguments):
        ...
        call = Call(
            "SYS_sigreturn", syscall_gadget, [SYS_sigreturn], abi.ABI.sigreturn()
        )
    ...
    def __load(self):
        ...
        valid = lambda insn: any(
            map(
                lambda pattern: pattern.match(insn),
        ...
        for elf in self.elfs:
            ...
            try:
                ...
                sys.argv = [
                    ...
                    elf.path,
                    "--only",
                    "sysenter|syscall|int|add|pop|leave|ret",
                    "--nojop",
                    "--multibr",
                ]
    ...
    def ret2plt(self, leak, ret=None):
    ...
    def ret2system(self, aling=True, bin_sh=None):
        ...
        elf = next(elf for elf in self.elfs)
        ret2system(self, elf, aling, bin_sh)
    ...
    def ret2dlresolve(self, dlresolve):
        ...
        call = Call(
            "[plt_init] " + dlresolve.symbol.decode(),
            plt_init,
            dlresolve.real_args,
            before=[reloc_index],
        )
    ...
    def __getattr__(self, attr):
        ...
        if attr in self.__dict__ or attr in self.BAD_ATTRS or attr.startswith("_"):
        ...
        if (
            attr == "jmp_esp"
            and context.arch == "i386"
            or attr == "jmp_rsp"
            and context.arch == "amd64"
        ):
            jmp_sp = {"i386": "jmp esp", "amd64": "jmp rsp"}[context.arch]
            ...
            return None
        mapping = {"int80": "int 0x80", "syscall": "syscall", "sysenter": "sysenter"}
        if attr in mapping:
